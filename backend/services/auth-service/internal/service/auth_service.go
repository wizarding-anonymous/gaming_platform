...
func (s *AuthService) Login(ctx context.Context, req models.LoginRequest) (*models.TokenPair, *models.User, string, error) {
	...
	if err != nil {
		if errors.Is(err, domainErrors.ErrUserNotFound) {
			...
			// Publish UserLoginFailedEvent for user_not_found
			loginFailedPayload := models.UserLoginFailedPayload{
				AttemptedAt:    time.Now(),
				IPAddress:      ipAddress,
				UserAgent:      userAgent,
				Reason:         "user_not_found",
				// UserID is empty as user is not found by email
			}
			subjectUserNotFound := "unknown_user_" + req.Email // Or a generic subject
			...
			// Publish UserLoginFailedEvent for user_not_found (This is the one to keep)
			loginFailedPayload := models.UserLoginFailedPayload{
				AttemptedAt:    time.Now(),
				IPAddress:      ipAddress,
				UserAgent:      userAgent,
			...
			subjectUserNotFound := "unknown_user_" + req.Email
			contentType := "application/json"
			if errPub := s.kafkaClient.PublishCloudEvent(ctx, s.cfg.Kafka.Producer.Topic, kafkaEvents.EventType(models.AuthUserLoginFailedV1), &subjectUserNotFound, &contentType, loginFailedPayload); errPub != nil {
				s.logger.Error("Failed to publish CloudEvent for user_not_found login failure", zap.Error(errPub))
			}
	...
	if !passwordMatch {
		...
		if fetchErr != nil {
			...
			// Publish UserLoginFailedEvent (credentials invalid, unable to fetch updated attempts)
            loginFailedPayload := models.UserLoginFailedPayload{
            ...
            if errPub := s.kafkaClient.PublishCloudEvent(ctx, s.cfg.Kafka.Producer.Topic, kafkaEvents.EventType(models.AuthUserLoginFailedV1), &subjectInvalidCreds, &contentType, loginFailedPayload); errPub != nil {
                s.logger.Error("Failed to publish CloudEvent for invalid_credentials login failure (user fetch failed)", zap.Error(errPub))
            }
			auditErrorDetails = map[string]interface{}{"reason": domainErrors.ErrInvalidCredentials.Error(), "attempted_email": req.Email}
			s.auditLogRecorder.RecordEvent(ctx, userIDForAudit, "user_login", models.AuditLogStatusFailure, userIDForAudit, models.AuditTargetTypeUser, auditErrorDetails, ipAddress, userAgent)
			return nil, nil, "", domainErrors.ErrInvalidCredentials
		...
		// Publish UserLoginFailedEvent (invalid_credentials)
		loginFailedPayload_InvalidCreds := models.UserLoginFailedPayload{
		...
		subjectFailedLogin_InvalidCreds := updatedUser.ID.String()
		contentType_InvalidCreds := "application/json"
		if errPub := s.kafkaClient.PublishCloudEvent(ctx, s.cfg.Kafka.Producer.Topic, kafkaEvents.EventType(models.AuthUserLoginFailedV1), &subjectFailedLogin_InvalidCreds, &contentType_InvalidCreds, loginFailedPayload_InvalidCreds); errPub != nil {
		...
		if updatedUser.FailedLoginAttempts >= s.cfg.Security.Lockout.MaxFailedAttempts {
			...
			if errLock := s.userRepo.UpdateLockout(ctx, updatedUser.ID, &lockoutUntil); errLock != nil {
				...
				accountLockedPayload := models.UserAccountLockedPayload{
					...
					TriggeringIP:   ipAddress,
					TriggeringUA:   userAgent,
					FailedAttempts: updatedUser.FailedLoginAttempts,
				}
				subjectAccountLocked := updatedUser.ID.String()
				if errPub := s.kafkaClient.PublishCloudEvent(ctx, s.cfg.Kafka.Producer.Topic, kafkaEvents.EventType(models.AuthUserAccountLockedV1), &subjectAccountLocked, &contentType_InvalidCreds, accountLockedPayload); errPub != nil {
			...
			// Audit log for lockout is recorded after publishing event.
			auditErrorDetails = map[string]interface{}{"reason": domainErrors.ErrUserLockedOut.Error(), "attempted_email": req.Email, "lockout_triggered": true, "failed_attempts": updatedUser.FailedLoginAttempts}
			s.auditLogRecorder.RecordEvent(ctx, userIDForAudit, "user_login", models.AuditLogStatusFailure, userIDForAudit, models.AuditTargetTypeUser, auditErrorDetails, ipAddress, userAgent)
			return nil, nil, "", domainErrors.ErrUserLockedOut
		}
		// Audit log for invalid credentials (not leading to lockout)
	...
	// ... (rest of the Login method from the previous read_files output)
	// This includes:
	// - UserStatusBlocked check
	// - EmailVerifiedAt check
	// - MFA Check
	// - ResetFailedLoginAttempts, UpdateLastLogin
	// - Session creation, TokenPair creation
	// - UserLoginSuccessEvent publishing
