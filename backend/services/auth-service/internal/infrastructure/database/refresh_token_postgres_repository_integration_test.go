// File: backend/services/auth-service/internal/infrastructure/database/refresh_token_postgres_repository_integration_test.go
package database
...
func TestRefreshTokenRepository_RevokeAllByUserID(t *testing.T) {
	...
	rt1u1Db, err := rtRepo.FindByID(ctx, rt1User1.ID); require.NoError(t, err)
	assert.NotNil(t, rt1u1Db.RevokedAt)
	rt2u1Db, err := rtRepo.FindByID(ctx, rt2User1.ID); require.NoError(t, err)
	assert.NotNil(t, rt2u1Db.RevokedAt)

	// Check user2's token is not revoked
	rt1u2Db, err := rtRepo.FindByID(ctx, rt1User2.ID); require.NoError(t, err)
...
func TestRefreshTokenRepository_DeleteExpired(t *testing.T) {
	...
	err := rtRepo.Create(ctx, rtExpired); require.NoError(t, err)
	...
	err = rtRepo.Create(ctx, rtActive); require.NoError(t, err)
