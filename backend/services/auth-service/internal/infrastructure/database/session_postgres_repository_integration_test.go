// File: backend/services/auth-service/internal/infrastructure/database/session_postgres_repository_integration_test.go
package database
...
func TestSessionRepository_DeleteAllByUserID(t *testing.T) {
	...
	err := sessionRepo.Create(ctx, session1User1); require.NoError(t, err)
	session2User1 := &models.Session{ID: uuid.New(), UserID: user1.ID, LastActivityAt: time.Now()}
	err = sessionRepo.Create(ctx, session2User1); require.NoError(t, err)

	// Create session for user2
	session1User2 := &models.Session{ID: uuid.New(), UserID: user2.ID, LastActivityAt: time.Now()}
	err = sessionRepo.Create(ctx, session1User2); require.NoError(t, err)
...
func TestSessionRepository_DeleteAllByUserID_ExcludeOne(t *testing.T) {
	...
	err := sessionRepo.Create(ctx, sessionToKeep); require.NoError(t, err)
	sessionToDelete := &models.Session{ID: uuid.New(), UserID: user1.ID, LastActivityAt: time.Now()}
	err = sessionRepo.Create(ctx, sessionToDelete); require.NoError(t, err)
...
func TestSessionRepository_DeleteExpired(t *testing.T) {
	...
	err := sessionRepo.Create(ctx, sessionExpired); require.NoError(t, err)
	...
	err = sessionRepo.Create(ctx, sessionActive); require.NoError(t, err)
