// File: backend/services/auth-service/tests/internal/infrastructure/database/mfa_secret_postgres_repository_integration_test.go
package database
...
func TestMFASecretRepository_DeleteAllForUser(t *testing.T) {
	...
	err := mfaSecretRepo.Create(ctx, secret1User1)
	require.NoError(t, err)
