// File: backend/services/auth-service/tests/internal/infrastructure/database/role_permissions_postgres_repository_integration_test.go
package database
...
func TestRolePermissionsRepository_Integration(t *testing.T) {
	...
	for i, p := range managerPermissions {
		permIDsForManager[i] = p.ID
	}
	...
	for i, p := range managerPermissionsAfterRemove {
		permIDsForManagerAfterRemove[i] = p.ID
	}
