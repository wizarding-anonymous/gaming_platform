// File: backend/services/auth-service/tests/internal/infrastructure/database/verification_code_postgres_repository_integration_test.go
package database
...
func TestVerificationCodeRepository_DeleteByUserIDAndType(t *testing.T) {
	...
	err := vcRepo.Create(ctx, vc1User1Email)
	require.NoError(t, err)
	...
	err = vcRepo.Create(ctx, vc2User1PassReset)
	require.NoError(t, err)
	...
	err = vcRepo.Create(ctx, vc1User2Email)
	require.NoError(t, err)
...
func TestVerificationCodeRepository_DeleteExpired(t *testing.T) {
	...
	err := vcRepo.Create(ctx, vcExpired)
	require.NoError(t, err)
	...
	err = vcRepo.Create(ctx, vcActive)
	require.NoError(t, err)
