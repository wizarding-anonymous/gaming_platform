// File: backend/services/auth-service/tests/internal/infrastructure/database/user_postgres_repository_integration_test.go
package database
...
func TestMain(m *testing.M) {
	...
	if dbPort == "" {
		dbPort = "5433"
	} // Often use a different port for test DB
	dbUser := os.Getenv("TEST_DB_USER")
	if dbUser == "" {
		dbUser = "test_auth_user"
	}
	dbPassword := os.Getenv("TEST_DB_PASSWORD")
	if dbPassword == "" {
		dbPassword = "test_auth_password"
	}
