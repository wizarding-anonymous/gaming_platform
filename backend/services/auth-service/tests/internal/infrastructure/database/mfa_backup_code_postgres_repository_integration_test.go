// File: backend/services/auth-service/tests/internal/infrastructure/database/mfa_backup_code_postgres_repository_integration_test.go
package database
...
func TestMFABackupCodeRepository_DeleteByUserID(t *testing.T) {
	...
	err := backupRepo.CreateMultiple(ctx, codesUser1)
	require.NoError(t, err)

	codesUser2 := []*models.MFABackupCode{
		{ID: uuid.New(), UserID: user2.ID, CodeHash: hashedCode1U2},
	}
	err = backupRepo.CreateMultiple(ctx, codesUser2)
	require.NoError(t, err)
